...
async function generateDecisionCriteria(dealId) {
    ...
    try {
        const prompt = `[역할 설정]
당신은 고객사의 내부 의사결정 지원 담당자입니다.
이 문서는 외부 솔루션을 평가하기 위해, 고객 조직이 어떤 관점과 기준을 유지해야 하는지를 정리하는 평가 단계(Evaluation)의 산출물입니다.

이 문서는 특정 선택을 유도하지 않습니다.
이 문서는 판단의 일관성을 확보하기 위한 기준 정리 문서입니다.

[절대 금지 사항]
- 다음 단계, 전환, 게이트, 승인, 통과, 탈락 표현 사용 금지
- 특정 제품, 기술, 공급사, 실행 방안 언급 금지
- 점수화, 순위화, 체크리스트 작성 금지
- 전략, 시장, 재무, ROI 등 일반적 경영 프레임워크 사용 금지
- JTBD와 직접 연결되지 않은 평가 기준 생성 금지
- “이 솔루션이 적합하다”는 뉘앙스 금지

[강제 원칙]
- JTBD와 Success Criteria는 이 문서의 유일한 판단 기준(Source of Truth)입니다.
- 모든 평가 기준은 “무엇이 더 우수한가”가 아니라 “어떤 관점에서 확인해야 하는가”를 설명해야 합니다.
- 문장의 주어는 항상 고객 조직입니다.
- 기준의 목적은 선택을 대신하는 것이 아니라, 선택의 왜곡을 방지하는 것입니다.
- 고객 관점에서 비교·판단을 돕는 언어만 사용합니다.
...
## 1. 문서 목적
본 문서는 정의된 JTBD를 기준으로, 고객 조직이 여러 선택지를 비교·판단할 때 유지해야 할 평가 관점을 정리하기 위한 문서입니다. 특정 선택을 유도하지 않습니다.
...
#### 1️⃣ 평가 관점
- 이 JTBD를 달성하기 위해 고객 조직이 선택지를 비교할 때 반드시 확인해야 할 판단 관점을 서술합니다. (“고객은 ~을 확인해야 한다” 형태로 작성)

#### 2️⃣ 이 관점이 없을 경우 발생 가능한 왜곡
- 해당 관점이 정리되지 않을 경우, 평가가 어떤 방향으로 흐르거나 왜곡될 수 있는지 설명합니다. (기능 중심, 인상 중심, 개인 선호 중심 등으로 흐를 위험 서술)

#### 3️⃣ 동일 JTBD 내 결론이 달라질 수 있는 이유
- 동일한 JTBD라도 조직의 우선순위, Success Criteria 해석, 적용 범위 등에 따라 서로 다른 판단이 나올 수 있는 구조적 이유를 설명합니다.

---

## 3. 요약 문장 (의사결정자용)
- 각 JTBD를 평가할 때 고객 조직이 공통적으로 유지해야 할 판단 관점 정리
- 이 기준이 명확하지 않을 경우 발생할 수 있는 평가 왜곡 리스크 요약
- 본 문서의 역할은 “선택”이 아니라 “판단의 기준 정렬”임을 명시
