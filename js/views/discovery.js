...
async function generateProblemDefinition(dealId) {
    ...
    try {
        const prompt = `[역할]
당신은 고객사의 내부 전략기획 담당자입니다.
외부 솔루션 관점이 아니라, 조직 내부 의사결정 리스크를 정리하는 역할을 수행합니다.
...
- 회사/시장 배경 설명 금지
- 기술, 제품, 도입 방식 언급 금지
- 개선 효과(ROI, 생산성 향상 등) 암시 금지
- 해결 방법 제시 금지
- 단, 조직이 지금 무엇을 명확히 정의해야 하는지는 반드시 제시할 것.

[작성 원칙]
- JTBD를 판단 과제로 재정의할 것
- 해결책이 아니라 판단 공백에 집중할 것
- 방치 시 발생하는 내부 의사결정 혼선을 설명할 것
- 각 문제 정의 끝에 → “조직은 이 사안에 대해 무엇을 명확히 정의해야 하는지 합의가 필요하다.” 포함

[출력 구조]
# 문제 정의서 (JTBD 기반)
## 문서 목적
본 문서는 JTBD가 조직 차원의 의사결정 리스크로 왜 중요한지 설명한다.
...
#### 조직이 명확히 정의해야 할 핵심 질문
- {{내용}}
→ “조직은 이 사안에 대해 무엇을 명확히 정의해야 하는지 합의가 필요하다.”

---

## 요약 문장
- JTBD들이 정의되지 않을 경우 발생하는 공통 판단 리스크 정리
