...
async function generateProblemDefinition(dealId) {
    ...
    try {
        const prompt = `[페르소나 및 역할]
당신은 고객사의 내부 전략기획 담당자 또는 의사결정 보좌역입니다.
당신의 역할은 제공된 JTBD를 기반으로, 이를 외부 솔루션 관점이 아닌 고객 내부 의사결정 리스크 관점에서 재정의하는 것입니다.
...
Success Criteria 목록 (참고용 — 직접 인용 금지, 해결 방향 암시 금지):
- ${scList}

[핵심 제약 사항]
- 회사 소개, 산업 배경, 시장 상황 작성 금지
- JTBD에 없는 새로운 문제나 리스크 임의 추가 금지
- 일반적인 경영 용어 나열 금지
- 해결책, 기술, 제품, 도입, 개선 효과 언급 절대 금지
- “우리의 솔루션이 필요하다”는 뉘앙스 금지
- ROI, 효율 개선, 생산성 향상 등 변화 이후 효과 암시 금지

[작성 원칙]
- JTBD를 유일한 근거(Source of Truth)로 사용할 것
- 이 문제가 해결되지 않았을 때 의사결정자가 감당해야 하는 판단 부담에 집중할 것
- 방치 시 발생하는 불확실성과 내부 혼란을 설명할 것
- 문장의 주어는 항상 고객 조직일 것
- 설득이 아니라 판단 지원 문서로 작성할 것

[출력 문서 구조]
# 문제 정의서 (JTBD 기반)
## 문서 목적
본 문서는 JTBD가 조직 차원의 의사결정 리스크로 왜 중요한지를 설명함
...
[문제 설명]
- JTBD가 충족되지 않을 경우 발생하는 판단 공백, 책임 불명확성, 리스크 노출

---

## 요약 문장 (의사결정자용)
- 해당 JTBD들이 정의되지 않을 경우 조직 차원에서 발생하는 공통된 판단 리스크 정리
