...
async function generateProblemDefinition(dealId) {
    ...
    try {
        const prompt = `[역할]
당신은 고객사의 내부 전략기획 담당자이자, 동시에 프리세일즈 관점에서 인식 단계를 설계하는 전문가입니다.
당신의 임무는 제공된 JTBD를 기반으로:
1. 이것이 실제 조직 차원의 문제임을 명확히 하고
2. 방치 시 논의 구조가 어떻게 왜곡되는지 설명하며
3. 솔루션 벤더가 인식 단계를 지원하기 위해 준비해야 할 논리·자료를 정리하는 것입니다.
해결책 제안이 아니라, 문제 확정 및 판단 구조 정리가 목적입니다.

[입력 데이터]
고객사명: ${clientName}
JTBD 목록 (유일한 근거, Source of Truth):
- ${jtbdList}

Success Criteria 목록 (참고용, 직접 인용 금지):
- ${scList}
...
- JTBD에 없는 새로운 문제 추가
- 회사/산업/시장 배경 설명
- 특정 기술, 제품, 기능, 도입 방식 언급
- 개선 효과(ROI, 생산성 향상 등) 직접 주장
- “우리 솔루션이 필요하다”는 뉘앙스

단, 반드시 포함합니다:
- 이 사안이 왜 “검토 이슈”가 아니라 “조직 문제”인지 명확히 규정할 것
- 방치 시 논의가 어떻게 왜곡되는지 구조적으로 설명할 것
- 벤더가 준비해야 할 판단 프레임·논리 구조·자료 유형을 정리할 것 (제품 설명이 아니라, 문제 정의를 돕는 자료)

[작성 원칙]
각 JTBD는 반드시 다음 3단 구조로 작성할 것:
1️⃣ 문제 확정 (Problem Confirmation): 이것이 단순 검토 항목이 아니라 조직 차원의 판단 공백임을 명확히 규정하고, 현재 무엇이 정의되지 않았는지 선명하게 표현.
2️⃣ 방치 시 구조 왜곡 (Structural Distortion): 정의되지 않은 상태로 논의가 진행될 경우 논의가 어떻게 왜곡되거나 축소될 수 있는지 설명 (예: 기술 중심 논의, 부서 갈등, 파일럿 중심 사고 등).
3️⃣ 인식 단계에서 벤더가 준비해야 할 사항 (Enablement Requirement): 전략적 판단 프레임, 의사결정 논리 구조, C-Level용 메시지 구조, 정의를 돕는 질문 세트, 비교·판단 기준 템플릿 등. (※ 기술적 해결책이나 제품 기능 설명은 절대 금지)

[출력 구조]
# 문제 정의서 (JTBD 기반)
## 1. 문서 목적
본 문서는 JTBD가 단순한 검토 사안이 아니라 조직 차원의 의사결정 문제임을 명확히 하고, 인식 단계에서 반드시 정의되어야 할 판단 과제를 정리하기 위한 문서임을 명시한다.
...
#### 3️⃣ 인식 단계에서 벤더가 준비해야 할 사항
- {{내용}}

---

## 3. 요약 문장 (의사결정자용)
- 상기 JTBD들이 정의되지 않을 경우 발생하는 공통된 판단 왜곡 요약
- 왜 지금 “문제 정의” 자체가 필요한지 명확히 정리
- 해결책 제안 금지
...
async function generateDecisionCriteria(dealId) {
    ...
    try {
        const prompt = `[역할 설정]
당신은 ${clientName}의 내부 의사결정 실무 책임자입니다.
이 문서는 외부 제안사를 설득하기 위한 문서가 아니라, 우리 조직이 성공 기준을 달성할 수 있는 제품을 선정하기 위해 내부적으로 정의하는 평가 기준 문서입니다.

이 문서는 PoC 또는 제안 요청(RFP) 직전 단계에서 작성되며, 제품을 비교·선별하기 위한 실제 판단 기준을 구체적으로 정의해야 합니다.

[문서 목적]
이 문서는 정의된 JTBD와 Success Criteria를 기준으로, 우리 조직이 어떤 조건을 충족하는 제품을 선택해야 하는지를 명확히 하기 위한 문서입니다.
이 문서는 결론을 내리지 않지만, 우리의 성공 기준을 구조적으로 달성할 수 없는 제품은 배제될 수 있도록 기준을 명확히 해야 합니다.
...
[강제 원칙]
- JTBD와 Success Criteria는 유일한 판단 기준(Source of Truth)입니다.
- 모든 평가 기준은 Success Criteria 달성 가능성과 직접 연결되어야 합니다.
- 추상적 표현(전략적 부합성, 혁신성, 시장성 등) 사용 금지
- 일반론 금지 (ROI가 중요하다 수준 금지)
- 특정 공급사/제품 이름 언급 금지
- 점수화·순위화 금지
- “좋다/나쁘다” 판단 금지
- 반드시 우리가 무엇을 확인해야 하는지 형태로 작성
- 기술, 데이터 구조, 통합 방식, 운영 적용 가능성 등은 구체적으로 포함
- PoC 단계에서 검증 가능해야 할 항목 포함

[평가 기준 작성 원칙]
각 Success Criteria에 대해 다음을 반드시 정의할 것:
1. 우리가 확인해야 할 기능적 조건
2. 우리가 확보해야 할 데이터 구조 및 측정 방식
3. 기존 시스템과의 연계 조건
4. 현업 적용 가능성 판단 기준
5. 파일럿 단계에서 검증해야 할 항목
6. 성공 기준 미달 가능성에 대한 사전 점검 요소

모든 기준은 아래 질문에 답해야 한다:
"이 제품은 우리의 성공 기준을 실제로 달성할 구조를 갖추고 있는가?"

[출력 문서 구조 – 강제]
# 평가 기준 정의서 (제품 선정 기준 · 고객 관점)

## 1. 문서 목적
본 문서는 정의된 JTBD와 Success Criteria를 기준으로, 우리 조직이 성공 기준을 달성할 수 있는 제품을 선정하기 위해 확인해야 할 구체적 판단 기준을 정리한다.

---

## 2. Success Criteria별 평가 기준 (반복)
### [연결된 JTBD]
- {{해당 JTBD 원문}}
...
#### 1️⃣ 우리가 확인해야 할 평가 기준
- **기능적 충족 조건**: 해당 수치를 달성하기 위해 제품이 반드시 제공해야 할 기능은 무엇인가?
- **데이터 및 측정 구조 조건**: 성공 여부를 어떻게 측정할 수 있는가? Before/After 비교가 가능한 구조인가? 데이터 자동 수집이 가능한가?
- **통합 및 기술 조건**: 기존 시스템과 어떤 방식으로 연계되는가? 추가 개발 또는 구조 변경이 필요한가? 보안 및 접근 통제는 우리 기준을 충족하는가?
- **운영 적용 조건**: 현업 사용자의 추가 업무 부담은 어느 수준인가? 초기 정착 가능성은 어떻게 판단할 것인가? 교육 및 운영 안정화까지의 예상 난이도는 어느 정도인가?
- **파일럿 검증 항목**: PoC 단계에서 반드시 검증해야 할 핵심 지표는 무엇인가? 성공 기준과 직접 연결되는 검증 방식은 무엇인가?
- **사전 리스크 점검 항목**: 성공 기준 달성이 어려울 수 있는 구조적 한계는 무엇인가? 조직 적용 저해 요인은 무엇인가? 측정 불가능 또는 데이터 왜곡 가능성은 없는가?

---

## 3. 요약 (의사결정자용)
- 우리의 제품 선정은 기능 비교가 아니라, 성공 기준을 실제로 달성할 수 있는 구조를 갖춘 제품을 식별하는 과정이다.
- 이 평가 기준은 선택을 대신하지 않지만, 우리의 성공 기준을 충족할 수 없는 제품을 사전에 걸러내기 위한 판단의 틀을 제공한다.
...
async function generateProjectSuccessGuide(dealId) {
    ...
    const clientName = deal.clientName || "고객사";
    const scList = Array.isArray(disc.sc) ? disc.sc.join("\n- ") : disc.sc;
    const solutionSummary = deal.solution || "선정된 솔루션";
    ...
    try {
        const prompt = `[역할 설정]
당신은 고객사의 프로젝트 성공을 지원하는 B2B 프리세일즈 전문가입니다.
이 문서는 계약 전·후 고객과 공유 가능한 **‘프로젝트 성공 가이드’**입니다.
이미 선정된 솔루션을 전제로, 프로젝트 성공을 가능하게 하는 조건을 정리합니다.

[문서의 목적]
이 문서는 구매 단계에서 합의된 Success Criteria를 기준으로,
- 프로젝트 성공을 가능하게 하는 전제 조건은 무엇인지
- 그 전제가 흔들릴 수 있는 지점은 어디인지
- 어떤 상태가 유지될 경우 관리 가능하다고 인식했는지
를 정리하는 문서입니다.
이 문서는 결정을 요구하지 않으며, 실행 계획이나 책임 분담을 정의하지 않습니다.

[절대 금지 사항]
- Go / No-Go, 평가, 승인, 통과, 탈락 등의 표현 사용 금지
- 계약 조건, 책임 배분, 일정 확정, 실행 계획 작성 금지
- “보장한다”, “확보한다”, “해결한다”와 같은 확정적 표현 금지
- 솔루션의 우월성 주장, 마케팅성 표현 금지

[허용 범위 – 중요]
구매 단계 특성상 다음은 허용됩니다:
- 선정된 솔루션의 구조, 기능, 아키텍처 언급
- 기술적 특성이 Success Criteria 유지와 어떻게 연결되는지 설명
- 운영 메커니즘 수준의 기술 설명
단, 반드시 다음 원칙을 따릅니다:
- 기능 설명이 아니라 성공 기준을 유지시키는 조건의 관점에서 서술
- “무엇을 해준다”가 아니라 “어떤 구조가 유지될 경우 관리 가능하다고 인식됨” 형태로 작성
- 홍보·설득 문서처럼 보이지 않도록 중립적 표현 유지

[강제 개념 규칙]
- Success Criteria는 이 문서의 유일한 출발점(Source of Truth)입니다.
- 각 SC는 반드시 다음 3가지로 재해석되어야 합니다:
  1. 프로젝트 성공을 가능하게 하는 전제 조건
  2. 해당 전제가 흔들릴 수 있는 리스크
  3. 어떤 기술적·운영적 상태가 유지될 경우 관리 가능하다고 인식했는지
- 리스크는 새로 창출하지 않습니다. 반드시 SC가 유지되지 않을 수 있는 상황을 뒤집어서 도출합니다.
- 대응 방안은 실행 계획이 아닙니다. “무엇을 하겠다”가 아니라 “어떤 조건이 유지될 경우 안정적으로 판단됨” 형태로 작성합니다.

[톤 가이드]
❌ 해결한다 / 보장한다 / 확보한다 / 문제 없다
❌ 당사 솔루션은 ~하다
❌ 강력하다 / 혁신적이다 / 우수하다
⭕ ~구조가 유지될 경우 관리 가능한 것으로 인식됨
⭕ ~기능이 SC와 직접적으로 연결되어 작동할 경우 안정적으로 판단됨
⭕ ~범위 내에서 운영될 경우 통제 가능하다고 인식됨

[입력 데이터]
고객사: ${clientName}
성공 기준(Success Criteria) 목록:
- ${scList}
선정된 솔루션 개요(구조/기능/아키텍처 요약): ${solutionSummary}

[출력 문서 구조 – 강제]
# 프로젝트 성공 가이드

## 1. 문서 목적
본 문서는 프로젝트가 성공적으로 진행되기 위해 합의된 Success Criteria를 기준으로, 해당 기준이 유지되기 위한 조건과 흔들릴 수 있는 지점에 대한 공통 인식을 정리한 가이드입니다. 특정 결정을 전제하거나 확정하지 않습니다.

---

## 2. 프로젝트 성공을 가능하게 하는 전제 조건
- Success Criteria를 프로젝트 성공 관점에서 재해석하여 목록화
- 선정된 솔루션의 구조·기능이 어떤 조건으로 작동할 때 해당 SC가 유지되는지 연결하여 설명

---

## 3. 예상되는 리스크
- 각 Success Criteria가 유지되지 않을 수 있는 상황을 중심으로 작성
- 기술적·운영적 조건이 불안정해질 수 있는 지점 명시 (성공 기준과 1:1 대응)

---

## 4. 리스크 대응에 대한 공통 인식
- 실행 계획이 아니라, 어떤 기술적·운영적 상태가 유지될 경우 관리 가능하다고 인식했는지 정리
- “~할 것이다” 표현 금지
- 성공 기준과 1:1 대응
